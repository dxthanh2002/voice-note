    ...
    ATListenerManager.bannerEventHandler.listen((value) {
      switch (value.bannerStatus) {
        ...
        case BannerStatus.bannerAdDidFinishLoading:
          ...
          //到达展示场景，展示前检查是否准备就绪
          ATBannerManager.bannerAdReady(placementID: value.placementID).then((
            isReady,
          ) {
  ...
  loadBannerAd() async {
    await ATBannerManager.loadBannerAd(
      ...
      extraMap: {
        ...
        ATBannerManager.getAdaptiveWidthKey(): TopSize()
            .getWidth(), //设置自适应尺寸，根据宽度来，部分平台有效
        ATBannerManager.getAdaptiveOrientationKey():
            ATBannerManager.adaptiveOrientationCurrent(), //自适应横竖屏，部分平台有效
      },
    );
  }
