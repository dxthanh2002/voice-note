...
class RewarderTool {
  ...
  //开启全自动加载
  void startLoadAutoRewardedVideoAd() async {
    //设置激励视频加载时透传参数(可选)
  ...
  hasRewardedVideoAdReady(String placementID) async {
    await ATRewardedManager.rewardedVideoReady(placementID: placementID).then((
      value,
    ) {
      print('flutter rewardedVideoReady: $value');
    });
  ...
  getRewardedVideoAdValidAds(String placementID) async {
    await ATRewardedManager.getRewardedVideoValidAds(
      ...
      print(
        'flutter getRewardedVideoValidAds: $value + placementID: + $Configuration.autoInterstitialPlacementID',
      );
    });
  }
